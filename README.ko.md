...
인벤토리가 아직 수집되지 않았다면 `vctl wg sync <gateway...>`를 먼저 실행합니다.
...
대시보드는 사이트 쌍마다 한 줄씩 보여줍니다. 줄을 선택하면 네트워크 → 게이트웨이 →
WireGuard → 게이트웨이 → 네트워크의 고정된 읽기 순서로 펼쳐집니다. 다이어그램에는
handshake 상태와 방향별 트래픽이 표시됩니다. 80칸 터미널이면 충분합니다. 더 좁으면
경로를 세로로 쌓습니다. 같은 사이트 쌍 사이의 터널 여러 개는 한 줄에 묶입니다. 좌우 키로
살펴볼 터널을 고릅니다. 소속을 모르는 엔드포인트는 따로 둡니다.

네트워크 이름과 주소는 네트워크 엔티티로 향하는 `attached-to` 관계가 명시돼 있을 때만
가져옵니다. 관계가 없으면 네트워크를 미확인으로 표시합니다. 반대편 peer의 AllowedIPs에
설정된 목적지가 있으면 별도로 보여줍니다. 관리용 IP, 인벤토리에서 추정한 /24,
경유 경로는 로컬 네트워크 소속의 증거로 치지 않습니다. handshake가 있어도 라우팅된
도달 가능성이 증명되지는 않습니다.

위/아래 또는 `j`/`k`로 사이트 연결을 선택하고 Enter로 다이어그램을 접거나 펼칩니다.
`d`는 기술 상세(인터페이스, AllowedIPs, 배치, 선언된 경로)를 엽니다. PageUp/PageDown으로
스크롤하고 `d`나 Esc로 돌아옵니다. `/`로 검색하고 Enter로 필터를 확정합니다. Esc로 지우고
`q`로 종료합니다. 목록을 필터해도 헤더는 전체 토폴로지를 요약합니다. 전송률은 최근 성공한
표본 두 개가 있어야 나옵니다. 터널마다 관측자 하나만 써서 이중 계산을 피합니다. 호스트
인자는 실시간 폴링 대상만 제한합니다. 저장된 토폴로지 전체는 계속 보입니다.
...
- **값은 요청하지 않으면 출력하지 않습니다.** 터미널에서 `get`은 뷰어를 엽니다. 필드가 나열되고 커서가 있는 행만 값이 보입니다. enter로 그 값을 클립보드에 복사합니다. 대체 화면에서 돌기 때문에 q로 나오면 스크롤백에 아무것도 남지 않습니다. 파이프로 넘기면 키 이름만 보이고 값은 가려집니다. 대개는 그것만 알면 됩니다. `--reveal`은 값을 모두 출력하고 `--field <key>`는 스크립트용으로 하나만 출력합니다. `-o json`에서 `data` 객체는 `--reveal`일 때만 들어갑니다. 자리표시자가 아니라 아예 빠집니다.
...
| `vctl kv get [word\|path] [--reveal] [--field <key>] [--version <n>]` | 위와 같은 방식으로 시크릿을 찾습니다. 터미널에서는 뷰어가 열립니다. ↑/↓로 필드를 오가면 커서가 있는 행만 값이 보이고, enter로 그 값을 클립보드에 복사하고, q로 나오면 화면에 아무것도 남지 않습니다. 파이프로 넘기면 키만 보이고 값은 가려집니다. `--reveal`은 값을 모두 출력하고 `--field`는 스크립트용으로 값 하나만 출력합니다. `-o json`은 `--reveal`일 때만 `data`를 담습니다 |
...
| `vctl list [--dc <dc>] [--wide] [--all-ips] [--drift]` | 인벤토리를 터미널 폭에 맞는 간결한 표로 표시합니다. `--wide`는 에이전트·운영 상태·SSH 사용자를 별도 열로 표시합니다. `--drift` 는 주 주소가 node-agent 가 보는 주소도 아니고 응답하지도 않는 호스트를 지목합니다. 기계가 옮겨간 뒤 낡아 버린 인벤토리인데, 주소 열이 합쳐져 있어 평소에는 안 보입니다. 후보만 실제로 두드려 봅니다 — 부동 IP 는 NAT 뒤 호스트가 원래 못 보는 주소이기 때문입니다. 점프 호스트를 거치는 호스트는 두드리지 않습니다. 내 장비에서 하는 dial 은 `JumpVia`를 따라가지 않아서입니다. 그런 호스트와 내 장비에 경로가 없는 후보는 미판정으로 따로 적습니다 |
...
mkdir -p .vctl
mkdir -p .vctl
cp config.example.yaml .vctl/config.yaml   # 그 다음 override할 값만 남기세요
```
...
이 패키지들은 `VCTL_TEST_DSN`이 가리키는 DB 하나를 같이 씁니다. `go test ./...`는 그 패키지들을 동시에 돌립니다. 그래서 테스트는 자기가 넣은 행에만 단언해야 합니다. 다른 패키지의 픽스처가 실행 도중에 생겼다 사라지기 때문입니다. 결과 집합을 통째로 비교하는 테스트는 코드가 아니라 스케줄러를 보고합니다. 그런 실패는 장비 부하에 따라 나왔다 안 나왔다 하고 코드 회귀처럼 보입니다.
