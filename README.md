...
| `vctl openstack explore [deployment]` | Full-screen browser: deployments on the left, the selected one's VMs or hosts on the right, `enter` for the same detail view `openstack host` / `vm show` print. `tab` moves between panes, `/` filters, `r` re-reads. Read-only, and it reads the database alone — nothing here contacts a control plane |
