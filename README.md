...
| `vctl wg sync\|graph\|monitor\|serve` | Collect and inspect WireGuard topology; `serve` shows live traffic, endpoint VMs, physical hosts, and host networks. `serve` also has a layered architecture view (`?view=layers`, or the LAYERS button) that draws the declared underlay — sites, farms, hosts, networks — under the tunnels, with which tunnel carries which network and where it is masqueraded; `graph --format json` writes that same picture, derived facts included, for scripts |
| `vctl wg endpoint list\|set\|rm` | Map a WireGuard public key to a VM/device identity and, for VMs, its physical inventory host |
| `vctl wg entity list\|set\|rm`, `vctl wg relation list\|set\|rm` | Declare the underlay the tunnels ride — sites, farms, physical hosts, VMs, networks, tunnels, edges, egress — and how they relate (`member-of`, `placed-on`, `attached-to`, `transits`, `carries`). A new farm or network is a row, not a code change: the dashboard lays it out from these rows |
