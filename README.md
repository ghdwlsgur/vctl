...
- **Values are never printed unless asked for.** On a terminal `get` opens a viewer: the fields are listed, only the row under the cursor shows its value, enter copies it to the clipboard, and because it runs in the alternate screen nothing stays in the scrollback after q. Piped, the keys are listed and the values masked, which is usually the question. `--reveal` prints the values; `--field <key>` prints one for a script. With `-o json` the `data` object is present only under `--reveal` — absent, not a placeholder.
...
| `vctl kv get [word\|path] [--reveal] [--field <key>] [--version <n>]` | Same resolution as the bare command. On a terminal it opens a viewer: ↑/↓ moves through the fields and only the row under the cursor shows its value, enter copies it to the clipboard, q leaves with nothing left on the screen. Piped, the keys are listed and the values masked; `--reveal` prints them all, `--field` prints one bare value for scripts. `-o json` carries `data` only with `--reveal` |
...
| `vctl kv set <path> [<key>=<value>...] [--rm <key>] [--replace]` | Write fields of a KV secret. Merges by default under check-and-set: fields you name are written, the rest kept, and a secret that changed between the read and the write is re-read and the edit reapplied. Values come from the argument, `@file`, or `-` (stdin); nothing written is printed back, field names only. Full path required — a word is never matched for a write. Gated by `kv-set` |
...
| `vctl list [--dc <dc>] [--wide] [--all-ips] [--drift]` | List inventory hosts in a compact responsive table; `--wide` separates agent, state, and SSH user columns. `--drift` names the hosts whose primary address is neither one the node-agent sees nor one that answers — an inventory gone stale under a machine that moved, which the merged address column otherwise hides. It probes the candidates, since a floating IP is legitimately an address a NAT'd host never sees. Hosts reached through a jump are not probed — a dial from your machine does not follow `JumpVia` — and are listed as unjudged, as are candidates your machine has no route to |
...
| `vctl edit [host] [flags]` | Change the fields `sync` will not overwrite — dc, ssh user, jump host, the primary address (`--ip`), extra IPs, hostname, and `--state active\|maintenance\|broken\|retired`. With no host, pick one from a list (←/→ filters by DC) |
...
| `vctl wg forget <host...>\|--gone [--dry-run]` | Drop a host's collected interfaces, peers and runtime status. Nothing else does: `wg sync` clears those rows only as the first half of replacing them, so a decommissioned gateway can never clear its own and keeps being drawn. `--gone` sweeps every collected host the inventory no longer has. Declared entities and annotations are left alone — they have their own `rm` |
...
| `vctl wg failover status\|to-standby\|to-primary\|hold` | The standby-hub switch each remote runs for itself: which hub is carrying its transit networks, both handshake ages, and — for any remote not at rest — that remote's own reasoning, read from the script rather than recomputed here. Forcing a move or freezing automatic switching needs the `wg-failover` grant |
...
mkdir -p .vctl
mkdir -p .vctl
cp config.example.yaml .vctl/config.yaml   # then trim to what you override
```
