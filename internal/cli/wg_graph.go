...
// pubIndex maps an interface public key to its (host, iface), so a peer's
// peer_pubkey can be resolved to the gateway that owns it.
func pubIndex(ifaces []store.WGInterfaceRow) map[string]nodeRef {
	m := make(map[string]nodeRef, len(ifaces))
	for _, i := range ifaces {
		m[i.PublicKey] = nodeRef{i.Host, i.Iface}
	}
	return m
}
