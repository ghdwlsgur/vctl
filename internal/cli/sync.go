...
func syncCmd(env cmdkit.Env) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runSync(cmd, env, opts)
		},
	}
	cmd.Flags().StringVar(&opts.prefix, "prefix", "sre", "only include ssh config aliases with this prefix")
	cmd.Flags().StringVar(&opts.path, "config", "", "ssh config path; defaults to ~/.ssh/config")
	cmd.Flags().BoolVar(&opts.doMigrate, "migrate", false, "run schema migrations before sync")
...
// runSync parses the ssh config, upserts the hosts it describes into the
// central inventory, and refreshes the local snapshot.
func runSync(cmd *cobra.Command, env cmdkit.Env, opts syncOptions) error {
	ctx := cmd.Context()
	a, err := env.App()
	if err != nil {
		return err
	...
	if opts.path == "" {
		opts.path = syncx.DefaultConfigPath()
	}
	blocks, err := syncx.Parse(opts.path)
	if err != nil {
		return err
	}
