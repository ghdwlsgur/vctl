...
func TestReachableAddressRanksWhatCanBeReached(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if got := ReachableAddress(tc.in, tc.nets); got != tc.want {
				t.Errorf("ReachableAddress = %q, want %q", got, tc.want)
			}
		})
	}
}
...
func TestReachableAddressReturnsAnAddressNotACell(t *testing.T) {
	...
	if got != "192.168.10.5" {
		t.Errorf("ReachableAddress = %q, want the bare address", got)
	}
}
