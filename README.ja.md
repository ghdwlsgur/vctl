...
インベントリを未収集なら、先に `vctl wg sync <gateway...>` を実行してください。
...
ダッシュボードはサイトの組ごとに 1 行を表示します。行を選ぶと、ネットワーク →
ゲートウェイ → WireGuard → ゲートウェイ → ネットワークという固定の読み順で展開されます。
図には handshake の状態と方向別のトラフィックが表示されます。80 桁の端末で足ります。
それより狭い場合は経路を縦に積みます。同じサイト間の複数トンネルは 1 行にまとまり、
左右キーで調べるトンネルを選びます。所属不明のエンドポイントは分けて表示します。

ネットワーク名とアドレスは、ネットワークエンティティへの明示的な `attached-to` 関係からのみ
取得します。関係がなければネットワークは未確認と表示します。対向 peer の AllowedIPs に
設定された宛先があれば別途示します。管理用 IP、インベントリから推定した /24、経由経路は
ローカルネットワーク所属の証拠として扱いません。handshake があっても、ルーティングされた
到達性の証明にはなりません。

上下または `j`/`k` でサイト接続を選び、Enter で図を折りたたみ・展開します。`d` で技術詳細
（インターフェース、AllowedIPs、配置、宣言された経路）を開きます。PageUp/PageDown で
スクロールし、`d` か Esc で戻ります。`/` で検索し、Enter でフィルタを確定します。Esc でクリア、
`q` で終了します。一覧をフィルタしてもヘッダーは全体のトポロジを要約します。転送レートには
直近の成功した 2 サンプルが必要で、二重計上を避けるためトンネルごとに観測者を 1 つだけ
使います。ホスト引数はライブポーリングの対象を絞るだけで、保存されたトポロジ全体は
引き続き表示されます。
...
`vctl kv` は `vctl login` が既に持っているトークンで Vault KV シークレットを読み、検索する。「あの資格情報はどこにあり、どのフィールドを持つか」に、別の CLI やコピーしたトークンなしで答える。引数なしなら `vctl ssh` と同じようにピッカーが開く。語を渡せばパスから探し、フルパスを渡せばそれをそのまま読む。パスを覚えて打ち直す必要はない。

```bash
vctl kv                                    # 見えるもの全体から選ぶ
vctl kv gitlab-albert                      # 語: その名のシークレット、複数ならピッカー
vctl kv search gitlab                      # マウント配下で "gitlab" を含むパス
vctl kv search oidc --under kv/teams/sre   # 一つのチーム配下に範囲を絞る
vctl kv list kv/teams/sre                  # 一階層: シークレットとフォルダ
vctl kv get kv/teams/sre/gitlab-albert     # キーは表示、値はマスク
TOKEN=$(vctl kv get kv/teams/sre/gitlab-albert --field token)
```

漏洩経路にならないよう、三つの規則を守る。

- **Vault がパスごとに決める。** これらのコマンドは権限を足しも引きもしない。一覧・読み取りできる範囲はトークンのポリシーが言う通りで、毎回サーバー側で検査される。検索中にトークンが一覧できないフォルダはスキップして数えるだけで、失敗にはしない。見えるものを受け取り、見えなかった分の注記を受け取る。
- **値は求められない限り出力しない。** ターミナルでは `get` はビューアを開く。フィールドが並び、カーソルのある行だけ値が見え、enter でその値をクリップボードにコピーする。代替スクリーンで動くので q で抜けるとスクロールバックに何も残らない。パイプに流すとキー名だけ表示され値はマスクされる。たいていはそれが問いだ。`--reveal` で値をすべて表示し、`--field <key>` はスクリプト向けに一つだけ出力する。`-o json` では `data` オブジェクトは `--reveal` のときだけ含まれる。プレースホルダではなく不在だ。
...
| `vctl kv [word\|path]` | `vctl ssh` がホストを受け取るのと同じ方法でシークレットを読む。フルパスはそのまま、語はマッチで探す。一致が一つならそれ・名前が完全一致するものがあればそれ・複数ならピッカー(←/→ でフォルダを絞る)・引数なしなら見えるもの全体から選ぶ。ターミナルがなければ曖昧な語は候補を列挙するエラーになる |
| `vctl kv get [word\|path] [--reveal] [--field <key>] [--version <n>]` | 上と同じ方法でシークレットを探す。ターミナルではビューアが開く。↑/↓ でフィールドを移ると、カーソルのある行だけ値が見え、enter でその値をクリップボードにコピーし、q で抜けると画面に何も残らない。パイプに流すとキーだけ表示され値はマスクされる。`--reveal` で値をすべて表示、`--field` はスクリプト向けに値を一つだけ出力する。`-o json` は `--reveal` のときだけ `data` を含む |
...
| `vctl list [--dc <dc>] [--wide] [--all-ips] [--drift]` | インベントリを端末幅に合わせた簡潔な表で表示する。`--wide` はエージェント・運用状態・SSH ユーザーを別の列に表示する。`--drift` は、プライマリアドレスが node-agent の見るアドレスでもなく応答もしないホストを指す。マシンが移った後に古びたインベントリで、アドレス列が統合されているため普段は見えない。候補だけ実際に叩く — フローティング IP は NAT 配下のホストが本来見られないアドレスだからだ。ジャンプホスト経由のホストは叩かない。手元のマシンからの dial は `JumpVia` を辿らないためだ。そうしたホストと、手元から経路のない候補は未判定として別に記す |
...
これらのパッケージは `VCTL_TEST_DSN` が指す 1 つの DB を共有し、`go test ./...` はそれらを同時に実行します。したがってテストは自分が投入した行だけに表明できます。他のパッケージのフィクスチャが実行中に現れたり消えたりするためです。結果セットを丸ごと比較するテストは、コードではなくスケジューラを報告していることになります。その失敗はマシンの負荷によって出たり出なかったりし、コードの回帰のように見えます。
